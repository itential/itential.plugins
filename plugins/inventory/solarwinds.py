...
# github.com/dalrrard/ansible-solarwinds-inventory-plugin and updated to be
...
description:
  - Pulls inventory information from an instance of Solarwinds NCM to use as
    the inventory for running an Ansible playbook.  This plugin will map the
    Solarwinds connection profile to Ansible variables.
...
options:
  plugin:
    ...
    required: true
    choices: ["itential.plugins.solarwinds"]
    ...
    def _populate(self) -> None:
        ...
        for item in _raw_inventory:
            ...
            if item.__class__.__name__ == "InventoryResponse":
                display.v(f"item {item.__dict__}")
                self.inventory.set_variable(host_name, "ansible_host", item.agent_ip)

                if getattr(item, "ansible_network_os"):
                    self.inventory.set_variable(host_name, "ansible_network_os", item.ansible_network_os)

                if getattr(item, "ansible_connection"):
                    self.inventory.set_variable(host_name, "ansible_connection", item.ansible_connection)
...
class QuerySolarwinds(Iterator[T]):
    ...
    def _get_connection_profile(
        ...
        display.vvv(f"HTTP Response is {response.status} {response.reason}")
        json_response: dict[str, Any] = json.load(response)
        if json_response:
            display.vvvvv(f"RESPONSE: {json_response}")
            data = json_response.get("ConnectionData") or {}

            # create the connection profile and map return values to the
            # ConnectionProfileResponse
            profile = ConnectionProfileResponse()
            profile.user_name = data.get("Username")
            profile.password = data.get("Password")
            profile.enable_password = data.get("EnablePassword")
            profile.ssh_port = data.get("SshPort")
    ...
    def _query_swis(self, cls_name: str, node_fields: list[str]) -> Iterator[T]:
        ...
        try:
            ...
            display.vvvvv(f"RESPONSE: {body}")
            self._json_inventory_response: list[dict[str, str | int]] = body["results"]
        except KeyError:
            raise AnsibleParserError(
    ...
    def _post_message(
        ...
        try:
            display.vvv(f"POST {complete_url}")
            display.vvvvv(f"BODY: {payload}")
